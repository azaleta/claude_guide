...
### 6.5.2 Dynamic Routing (动态路由)

在更高级的系统中，Router 是智能的。它像一个项目经理，动态决定要把任务派给谁。

```mermaid
graph TD
    UserQuery[用户需求] --> Router{PM Agent}
    
    Router -->|涉及数据库| SQLSkill
    Router -->|涉及画图| ChartSkill
    Router -->|复杂逻辑| PythonSkill
...
### 6.5.4 团队协作与共享 (The Skill Bazaar)
...
下一章，我们将进入深水区，探讨 Claude 在软件工程领域的终极形态。
