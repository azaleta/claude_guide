...
**Skills 是 Anthropic 为 Claude 设计的一种模块化、可复用的专业知识与能力封装。**

### 6.1.1 Skills vs Prompts vs Tools

很容易混淆这三个概念，需要理清它们的关系：

| 概念                | 类比                   | 作用             | 定义方式                   |
| :---------------- | :------------------- | :------------- | :--------------------- |
| **System Prompt** | **人设** (Personality) | 决定整体风格和边界      | 单一文本，针对整个会话            |
| **Tools (MCP)**   | **手/口** (Apparatus)  | 连接外部系统执行操作     | JSON Schema 定义函数       |
| **Skills**        | **专业能力** (Expertise) | 封装特定任务的最佳实践和流程 | **提示词 + 工具 + 文档** 的集合包 |
...
如果把所有规则都写在 System Prompt 里，上下文窗口 (Context Window) 很快就爆了。
...
理解了 Skills 的概念，接下来看看一个标准的 Skill 到底长什么样？它的内部文件结构是如何组织的？
