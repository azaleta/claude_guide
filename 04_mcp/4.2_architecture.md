...
MCP 并不仅仅是一个简单的 API 定义，它是一套完整的分布式系统协议。要熟练使用 MCP，我们需要深入理解其架构组件及通信机制。
...
#### 1.1 MCP Client (Host)
**Client 是发起方**。通常是 Claude Desktop App、IDE (如 Cursor) 或你自己编写的 AI Agent。
...
### 2. 三大核心原语 (The Three Pillars)

MCP 协议定义了三种主要的能力交互方式。

#### 2.1 Resources (资源) —— "读取 Context"
...
理论知识已经具备，现在我们动手实战。我们将配置 Claude Desktop 来连接一个本地的 SQLite 数据库。
