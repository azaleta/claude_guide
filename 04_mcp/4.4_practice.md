...
单纯地连接一个 Server 只是第一步。MCP 的真正威力在于**组合**。
当 Claude 同时连接了“眼睛”（搜索）、“记忆”（数据库）和“双手”（GitHub）时，它就不再是一个聊天机器人，而是一个全栈工程师。

本节将通过三个典型场景，展示如何组合多个 MCP Server 来完成复杂任务。

### 1. 场景一：全自动代码审查员 (The AI Reviewer)
...
**Result**: 你桌面上多了一份文件，无需离开对话框。

### 2. 场景二：智能数据分析师 (The Data Analyst)
...
现在你已经体验了使用现成的 Server，但在企业中，你往往需要通过 MCP 暴露**内部的私有 API**。这就需要我们自己动手编写 MCP Server 了。
