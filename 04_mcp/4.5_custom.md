...
虽然社区提供了很多现成的 Server，但在企业应用中，你往往需要让 Claude 连接你的**内部 API**、**私有数据库**或者**自研系统**。

本节将教你用 Python 和 TypeScript 两种语言，快速开发一个自定义的 MCP Server。

### 1. 极速入门：FastMCP (Python)

如果你是 Python 开发者，Anthropic 官方提供了一个名为 `fastmcp`（包含在 `mcp` 库中）的高级封装，它可以让你像写 FastAPI 一样简单地写 MCP Server。
...
#### 1.3 本地测试
你可以使用 MCP Inspector 直接运行它：
```bash
npx @modelcontextprotocol/inspector python weather_server.py
```
这会弹出一个网页，你可以在上面看到 `get_weather` 工具，点击运行，还能看到 JSON 交互日志。

### 2. 企业级开发：TypeScript SDK

如果你是 Node.js 开发者，可以使用官方的 TypeScript SDK。这通常用于构建生产环境级别、基于 Docker 的 Server。
...
恭喜！你现在不仅会使用 MCP，还会创造 MCP。
Claude 现在已经连接上了你的数据。接下来，让我们玩点更刺激的——让 Claude 自己看屏幕，像人一样操作电脑。
