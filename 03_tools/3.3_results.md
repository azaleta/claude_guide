...
### 2. 并行工具执行 (Parallel Tool Use)

如上例所示，Claude 3.5 Sonnet 等先进模型具有强大的**并行调用能力**。它可以在一次回复中请求执行多个动作。

**关键规则**：如果模型一次性发出了 3 个 tool_use 请求，你必须在下一次交互中，一次性返回这 3 个请求对应的 `tool_result`。如果遗漏任何一个，会引发 API 错误。

#### 2.1 执行策略
对于并行请求，推荐使用异步编程（如图 Python 的 `asyncio`）并发执行，以减少总等待时间。

### 3. 构造工具结果 (Tool Results)

执行完工具逻辑后，你需要构造一个特定格式的 `tool_result` 消息块。
...
这是开发者最容易出错的地方。
为了让 Claude 理解上下文，你必须维护完整的对话历史链条。
...
**错误示范**：
如果在第3步发送结果时，如果你忘记在 `messages` 列表中包含第2步 Assistant 的原始请求消息，API 会报错，因为它找不到 `tool_use_id` 的来源。
...
掌握了请求与反馈的机制后，我们来看看如何将多个工具组合起来，实现复杂的任务编排。
