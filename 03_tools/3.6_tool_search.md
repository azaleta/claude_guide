...
### 1. 大规模工具库的挑战：上下文污染与 Token 黑洞

随着 Agent 能力的增长，我们很快会面临一个棘手的问题：**工具数量爆炸**。
一个成熟的企业级助手可能集成了 Jira、GitHub、Salesforce、AWS 和内部数据库，可用工具轻易超过 100 个。

如果你尝试将这 100 个工具的完整定义（名称+描述+参数Schema）全部塞进 API 请求：
...
### 2. 核心机制：按需加载 (Deferred Loading)

工具搜索的工作流程打破了传统的“一次性全部提供”模式：

1.  **定义**：在你的 `tools` 列表中，你可以提供数千个工具，但将绝大多数标记为 `defer_loading: true`。
2.  **隐藏**：对于这些延迟加载的工具，Claude **初始时看不到它们的完整定义**。它只知道“这里有很多关于数据库和代码的工具”。
3.  **索书号**：不仅如此，为了帮助 Claude 找到它们，你会提供一个特殊的**工具搜索工具**。
...
Anthropic API 提供了原生的搜索能力，这意味着你不需要自己写向量数据库来实现工具检索（虽然你完全可以这么做）。

#### 3.1 内容搜索 (BM25 Variant)
这是最通用的搜索模式，基于关键词匹配。
...
总计 100+ 工具。如果没有工具搜索，每次对话的 Context 都会爆满。有了工具搜索，你可以轻松挂载所有这些服务器，让 Claude 成为真正的全能 Agent，而只在真正需要“读文件”时才加载文件系统的工具定义。
...
随着工具的增多，我们不仅面临管理的挑战，还面临连接的挑战。如何标准化地连接本地文件、远程服务和各类数据库？
