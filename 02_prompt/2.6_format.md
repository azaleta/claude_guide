...
对于开发者而言，最令人头也就是 Claude 回复了一堆 *"Here is the JSON you requested..."* 的废话，或者在 JSON 里加了注释导致 `JSON.parse` 报错。
...
在 Chat Completions API 中，通常 `Assistant` 的消息是由模型生成的。但在 Claude API 中，**你可以手动代替 Assistant 说“第一句话”**。

这就像是你在它的嘴边放了一句话，强迫它接下去。
...
const fullJson = "{" + response.content[0].text; // 记得把你预填充的字符拼回去！
```
...
### 2. 移除“废话” (Skip Preamble)

如果你无法使用 API 的 Prefill 功能（例如在使用某些第三方客户端），你可以通过 Prompt 咒语来减少废话。
...
这是 API 层面的核武器。你可以告诉 API：“一看到某个特定字符，就立即切断生成”。
...
现在，我们掌握了提示工程的所有组件：基础原则、XML 结构、System Prompt、Few-Shot、CoT 和格式控制。
但是，怎么知道我们写的 Prompt 到底好不好？如何科学地改进它？
