...
通过向 Claude 展示几个“完美案例”，你可以无需费力解释抽象规则，就能让模型迅速领悟你的意图。这被称为 **In-Context Learning (上下文学习)**。

### 1. 原理：从模仿到泛化

LLM 是强大的模式识别引擎。当你提供示例（Shots）时，Claude 实际上在做两件事：
...
#### 2.2 黄金法则：多样性覆盖 (Diversity)
你的示例集必须覆盖任务的**边界情况 (Corner Cases)**。
...
#### 3.1 格式强化 (Format Enforcement)
有时即使你写了 "Output valid JSON"，模型还是会啰嗦 "Here is the JSON..."。
...
#### 3.2 思维链演示 (CoT via Few-Shot)
如果你希望 Claude 在推理时必须展示步骤，不要只在 System Prompt 里说。**在示例里直接写出来。**
...
### 4. Many-Shot Prompting (多样本提示)

这是 Claude 长上下文能力的特有玩法。
如果你的任务很难（例如：将自然语言转为一种极其晦涩的内部 DSL 查询语言），提供 3 个例子根本不够。
你可以利用 Claude 的 200k 窗口，一次性塞入 **50-100 个** 高质量的 Input-Output 对。

**效果惊人**：根据 Anthropic 的研究，Many-Shot 在某些复杂任务上的表现可以媲美经过 Fine-tuning（微调）的模型，但无需任何训练成本。

### 5. 动态示例库 (RAG for Few-Shot)

在生产环境中，你可能有 1000 个完美的示例，但不能每次都全部发给模型（Token太贵）。
最佳实践是建立一个**动态示例库**：
...
掌握了 Few-Shot，你已经能够解决 90% 的格式和风格问题。但面对复杂的逻辑推理难题，我们还需要一把更锋利的武器 —— 让 AI “慢下来思考”。
