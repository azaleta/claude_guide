...
在上一节中，我们学到了 Prompt 的核心是“消除歧义”。当 Prompt 只有几行时，自然语言就够了。但当你需要处理复杂的长文本、多步骤任务或需要极高稳定性的输出时，自然语言的边界就变得模糊了。

这时，你需要一种更严谨的语法。对于 Claude 来说，这种语法就是 **XML**。

### 1. 为什么是 XML？

你可能会问：*“为什么不是 JSON？为什么不是 Markdown？”*
...
3.  **解析便利**：Claude 生成的 XML 也可以被你的代码轻松地通过正则或 XML Parser 提取。
...
#### 2.1 推荐的标签词汇表
虽然你可以随意发明标签（如 `<aaa>`），但使用语义化的英文单词效果最好：
...
```
Claude 能够完美理解这些属性。比如你可以指令它：“请只总结 `status="final"` 的文档。”
...
3.  **易于提取**：你的后端代码可以只正则提取 `<answer>(.*?)</answer>` 之间的内容展示给用户，而隐藏 `<thinking>` 过程。

### 5. 防止 Prompt Injection (提示词注入)

假如你的应用是让 Claude 翻译用户输入的文本。
...
掌握了 XML，你就掌握了与 Claude 进行长文本、高复杂度沟通的通用语言。接下来，我们将把这些技巧应用到提示词中最关键的部分——**System Prompt**。
